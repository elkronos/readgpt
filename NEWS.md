...
* **A review that reads like a review: citations by name, a reference list, and
  ...
  of the thing people need to publish.
...
* **`preview`: the reader that decides how to read before reading.** Every
  ...
  mention defaults to **read**, never to skip, because silence must not be able
  to lose a document's contents. When no usable plan comes back at all, the reader
...
* **`gr_records()` and `gr_search()`: the review starts at the search, not at a
  ...
  than values a model read off a title page, which is the loosest guarantee in
  the pipeline, for the one part of a citation that must be exactly right. The
...
* **The `claims` protocol: a schema coded so studies can be compared, not only
  ...
  trusted. `effect` stays a string deliberately, because effect metrics are not
  ...
  strongest thing a review can say: that a disagreement about effect size is a
  disagreement about measurement. `limitation` is the one the *authors*
...
* **`iterative` was cutting the wrong end of what it had gathered, and never
  ...
  support from text no model had been shown. Only what reached the prompt is
  reported now, `notes$chunks_dropped` says how much did not, and a run
...
* **A provider that reports no usable token count falls back instead of
  ...
  *original*. `clone$set_turns()` emptied the caller's turns and left the clone's
  untouched. That is exactly backwards, and it made the adapter look as though it
...
* **A misspelt reader or segmenter setting is no longer swallowed in silence.**
  ...
  through, because the escape hatch has to stay open for a field that is new,
...
* **`parallel = TRUE` no longer under-reports the run it speeds up.** A worker
  ...
  same run made sequentially reports: the same calls, steps, tokens and cost.
...
* **A new reader, `extract`.** Traversal signature `all|N+conflicts|none`:
  ...
  chunks agree, so it costs a call only where a document contradicts itself.
  `resolve = "model"` adjudicates those, `resolve = "first"` (the
...
* **Synthesis.** `gr_synthesise()` writes the review up from the extraction
  table, one section per call, against the outline the protocol fixed in
  advance, so the structure is not shaped by whatever happened to be found, and a
  ...
  not there is reported and marks the section partial. It is the same check
...
* **A quote now cites the page it is on.** A chunk is packed from several
  units, and a chunk packed from two pages reported the *first* one: right for
  its opening sentence, wrong for everything after it, and wrong in
  ...
  several pages, or not found at all, is left alone, since the first hit would
...
* `gr_read_many()` returns `$sources`: the sources as they were read, aligned
  row for row with `summary`. `summary$document` is a display label (a basename,
  made unique with a suffix when two folders hold the same filename), and there
  is no way back from it to a file, which is what any caller feeding
...
* **Quoted evidence is checked against the document.** `ans$evidence` is what an
  ...
  longest consecutive run in the source. It is a run measure rather than word
...
* **An embedder registry.** `gr_register_embedder()` and `gr_embedders()` make
  ...
  **deterministic**. That closes the replay gap. A replay now reproduces a run's
  ...
  the replay uses the same one. Both conditions are checked against the embedder
  the trace recorded. Determinism alone is not enough: replaying an
...
* **One question, many documents.** `gr_read_many()` runs one recipe over a
  ...
  returns one row per document: `answer`, `not_found`, `partial`,
...
* **`gr_trace_cost()`** prices a run using each step's own model and counts only
  the calls that were issued. A call served from a cache or a replay spent
  nothing however large its prompt was. One row per model; an unpriced
...
* **A trace could not survive being written to a file.** `jsonlite` escapes
  ...
  prompt, response and error text with `mark_utf8()`, which labels and converts
...
* **The same document produced different results on different machines.**
  ...
  counting bytes, charging a one-codepoint character as three, a 50% token
  swing on the affected line. Token counts, chunk boundaries, budgets and
